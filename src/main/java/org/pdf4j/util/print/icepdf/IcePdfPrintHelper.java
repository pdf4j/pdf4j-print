...
import org.pdf4j.util.print.PrintHelper;
...
/**
 * <p>The <code>IcePdfPrintHelper</code> class is utility class implements 
 * <code>PrintHelper</code> to aid developers in printing PDF document content.  
 * The IcePdfPrintHelper takes advantage of the Pageable and Printable 
 * interfaces available in Java 2.</p>
 *
 * @author Uma
 */
public class IcePdfPrintHelper implements PrintHelper {

    private static final Logger logger =
            Logger.getLogger(IcePdfPrintHelper.class.toString());
     ...
     * @param rotation
     */
    public IcePdfPrintHelper(Container container, PageTree pageTree, int rotation) {
     ...
     */
    public IcePdfPrintHelper(Container container, PageTree pageTree,
        ...
        if (logger.isLoggable(Level.FINE)) {
            logger.log(Level.FINE, "Paper Size: {0} {1} x {2}", new Object[]{paperSizeName.getName(), size[0], size[1]});
        }
    }
     ...
     */
    @Override
    public final boolean setupPrintService(int startPage,
                                     int endPage,
                                     int copies,
...
//        services = new PrintService[]{services[0]};
    ...
    private PrintService[] lookForPrintServices() {
        PrintService[] lservices = PrintServiceLookup.lookupPrintServices(
        ...
        if (defaultService != null && lservices.length > 1) {
            PrintService lprintService;
            for (int i = 1, max = lservices.length; i < max; i++) {
                lprintService = lservices[i];
                if (lprintService.equals(defaultService)) {
                    // found the default printer, now swap it with the first index.
                    PrintService tmp = lservices[0];
                    lservices[0] = defaultService;
                    lservices[i] = tmp;
                    break;
                }
            }
